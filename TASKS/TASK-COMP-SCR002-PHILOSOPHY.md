...
- **TASKS/00_TASK_LIST.md 참조:** Seq 16

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 16)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 소개·철학 좌우 분할은(는) SCR-002(`/about`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr002/PhilosophySplit.tsx`

## Functional AC

자기소개·여행을 시작한 이유·여행 철학 2~4개 문단

## Visual AC

좌: 이미지, 우: 문단(Desktop)/세로 스택(Mobile)
...
- **TC-01:** 자기소개·여행을 시작한 이유·여행 철학 2~4개 문단 여부를 확인한다.
