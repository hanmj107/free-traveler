...
- **TASKS/00_TASK_LIST.md 참조:** Seq 27

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 27)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. Intro CTA Banner은(는) SCR-004(`/mates`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr004/IntroCtaBanner.tsx`

## Functional AC

"동행 모집글 작성하기" → `/travel-tools` 동행 구하기 탭
...
- **TC-01:** "동행 모집글 작성하기" → `/travel-tools` 동행 구하기 탭 여부를 확인한다.
