...
- **TASKS/00_TASK_LIST.md 참조:** Seq 22

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 22)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 항공 조건 입력·검증·요약·외부이동은(는) SCR-003(`/travel-tools`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr003/FlightForm.tsx`

## Functional AC

국가·지역·출발일·귀국일 필수값 미입력 시 이동 버튼 비활성화+필드별 오류(FLIGHT-001), 과거 출발일/귀국일<출발일 차단(FLIGHT-002), 입력 요약+비전달 고지(FLIGHT-003), "항공편 보러 가기"는 설정된 외부 URL을 새 탭으로 이동(FLIGHT-004), URL 미설정/오류 시 이동 불가 안내+재시도(FLIGHT-005)

## Visual AC

좌: 폼, 우: 실시간 요약(Desktop) / 세로 스택(Mobile)

## Security/Privacy AC

입력값 서버·쿼리·쿠키 미전송(FLIGHT-006), 외부 이동 `noopener noreferrer`(SEC-002), 오류 메시지 `aria-describedby` 연결(ACC-005)

## Test Cases

- **TC-01:** 국가·지역·출발일·귀국일 필수값 미입력 시 이동 버튼 비활성화+필드별 오류(FLIGHT-001) 여부를 확인한다.
- **TC-02:** 과거 출발일/귀국일<출발일 차단(FLIGHT-002) 여부를 확인한다.
- **TC-03:** 입력 요약+비전달 고지(FLIGHT-003) 여부를 확인한다.
- **TC-04:** "항공편 보러 가기"는 설정된 외부 URL을 새 탭으로 이동(FLIGHT-004) 여부를 확인한다.
- **TC-05:** URL 미설정/오류 시 이동 불가 안내+재시도(FLIGHT-005) 여부를 확인한다.
