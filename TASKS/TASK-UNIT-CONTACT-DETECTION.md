...
- **TASKS/00_TASK_LIST.md 참조:** Seq 54

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 54)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 연락처 탐지 Unit Test은(는) SCR-003에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `tests/unit/contactDetection.spec.ts`

## Functional AC

전화번호·메신저 ID 기본 패턴 포함 시 제출 차단 검증(정상 텍스트는 통과)
...
- **TC-01:** 전화번호·메신저 ID 기본 패턴 포함 시 제출 차단 검증(정상 텍스트는 통과) 여부를 확인한다.
- **TC-02:** 위 시나리오가 실패(회귀)할 경우 CI에서 실패로 표시되는지 확인한다.
