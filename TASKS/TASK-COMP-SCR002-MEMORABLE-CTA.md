...
- **TASKS/00_TASK_LIST.md 참조:** Seq 20

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 20)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 기억에 남는 여행지 4개 + CTA Banner은(는) SCR-002(`/about`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr002/MemorableDestinations.tsx`

## Functional AC

**정확히 4개** 카드(→ SCR-001 상세 Drawer 이동) + "여행 조건 정리하기"(`/travel-tools`)·"동행 찾아보기"(`/mates`) 버튼

## Visual AC

Card Grid 4 + CTA Banner
...
- **TC-01:** **정확히 4개** 카드(→ SCR-001 상세 Drawer 이동) + "여행 조건 정리하기"(`/travel-tools`)·"동행 찾아보기"(`/mates`) 버튼 여부를 확인한다.
