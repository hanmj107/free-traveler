...
- **TASKS/00_TASK_LIST.md 참조:** Seq 41

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 41)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 대표 소개 정적 데이터은(는) SCR-002에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
대표 수치(50+/30+)·Timeline 6개 이상·방문국가 30개국·Gallery 8장 이상(alt+캡션)·기억에 남는 여행지 4개 단일 소스. **정적 데이터로만 구현**(구현 방법 고정)
...
- **TC-01:** 대표 수치(50+/30+)·Timeline 6개 이상·방문국가 30개국·Gallery 8장 이상(alt+캡션)·기억에 남는 여행지 4개 단일 소스 여부를 확인한다.
- **TC-02:** **정적 데이터로만 구현**(구현 방법 고정) 여부를 확인한다.
