...
- **TASKS/00_TASK_LIST.md 참조:** Seq 39

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 39)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 여행지 정적 데이터은(는) SCR-001에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
국내 10개 이상·해외 15개국 30개 도시 이상, §3-4 11개 필수 항목을 TypeScript 타입으로 강제(빌드 실패로 누락 방지). **정적 데이터로만 구현하며 DB 테이블을 만들지 않는다**(구현 방법 고정)
...
- **TC-01:** 국내 10개 이상·해외 15개국 30개 도시 이상 여부를 확인한다.
- **TC-02:** §3-4 11개 필수 항목을 TypeScript 타입으로 강제(빌드 실패로 누락 방지) 여부를 확인한다.
- **TC-03:** **정적 데이터로만 구현하며 DB 테이블을 만들지 않는다**(구현 방법 고정) 여부를 확인한다.
