...
- **TASKS/00_TASK_LIST.md 참조:** Seq 35

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 35)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 로그인/회원가입/재설정 + 성인확인은(는) SCR-005(`/account`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr005/AuthForms.tsx`

## Functional AC

이메일 로그인/회원가입(닉네임+성인확인 체크박스)/비밀번호 재설정, 성인확인은 자기신고 체크박스+확인 시각 저장(생년월일 미저장)

## Visual AC

Guest 전용 3개 카드 + 로그인 후 기능 Chip 3개

## Security/Privacy AC

이메일·닉네임·성인확인여부만 수집(PRIV-001), 생년월일 미저장(PRIV-002)

## Test Cases

- **TC-01:** 이메일 로그인/회원가입(닉네임+성인확인 체크박스)/비밀번호 재설정 여부를 확인한다.
- **TC-02:** 성인확인은 자기신고 체크박스+확인 시각 저장(생년월일 미저장) 여부를 확인한다.
