...
- **Category:** CI_DEPLOY (CI/Deploy 확인)
- **Priority:** P1
- **Implementation Status:** IN_SCOPE_PENDING
- **TASKS/00_TASK_LIST.md 참조:** Seq 61

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 61)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. Vercel/Supabase 배포 확인은(는) 여러 화면에 걸친 공통/백엔드 계층에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.

## Project Scope

**IMPLEMENT** — `docs/06_SRS_UIUX_REVISED.md`, `docs/PROJECT_SCOPE.md`에서 이번 범위 구현 대상으로 확정됨.

`docs/PROJECT_SCOPE.md` §8 "배포" 원칙: Vercel 배포만 사용하고 별도 EC2/AWS 인프라, 업타임 모니터링, 부하 테스트 체계를 구축하지 않는다.
...
- `TASKS/checklists/release-vercel-supabase.md`

## Functional AC

배포 URL 5개 Route 전부 HTTPS 접근 확인(SEC-001), Supabase 연결 차단 상태를 인위적으로 만들어 SCR-001/SCR-002 정적 콘텐츠가 계속 열람되는지 브라우저로 확인(AVAIL-003) — **브라우저 확인 필요 항목을 본 Release Check에 연결**(원칙 4)
...
- **TC-01:** 배포 URL 5개 Route 전부 HTTPS 접근 확인(SEC-001) 여부를 확인한다.
- **TC-02:** Supabase 연결 차단 상태를 인위적으로 만들어 SCR-001/SCR-002 정적 콘텐츠가 계속 열람되는지 브라우저로 확인(AVAIL-003) — **브라우저 확인 필요 항목을 본 Release Check에 연결**(원칙 4) 여부를 확인한다.
...
- EC2·AWS 등 별도 인프라를 구성하지 않는다. 자동 Merge/병합 파이프라인을 만들지 않는다.
