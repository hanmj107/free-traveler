...
- **TASKS/00_TASK_LIST.md 참조:** Seq 17

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 17)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 여행 Timeline 6개 이상은(는) SCR-002(`/about`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr002/TravelTimeline.tsx`

## Functional AC

연도별 항목 **최소 6개**(연도·여행지·한 줄 설명), 6개 미만 렌더링 금지
...
- **TC-01:** 연도별 항목 **최소 6개**(연도·여행지·한 줄 설명) 여부를 확인한다.
- **TC-02:** 6개 미만 렌더링 금지 여부를 확인한다.
