...
- **TASKS/00_TASK_LIST.md 참조:** Seq 49

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 49)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 참가 요청 생성/승인/거절 API은(는) 여러 화면에 걸친 공통/백엔드 계층에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- **Screen:** N/A(화면 비종속)
- **Route:** N/A
- **Page Entry:** `src/app/api/mates/[id]/applications/route.ts`
...
- `src/app/api/mates/[id]/applications/route.ts`
- `src/app/api/applications/[id]/route.ts`

## Functional AC

`(post_id, applicant_user_id)` PENDING/APPROVED 중복 시 409, 작성자만 승인/거절 가능, 승인/거절은 Toast로 알림 대체(이메일 미사용, 구현 방법 고정)
...
신청자 본인/글 작성자만 상태 변경 가능

## Test Cases

- **TC-01:** `(post_id, applicant_user_id)` PENDING/APPROVED 중복 시 409 여부를 확인한다.
- **TC-02:** 작성자만 승인/거절 가능 여부를 확인한다.
- **TC-03:** 승인/거절은 Toast로 알림 대체(이메일 미사용, 구현 방법 고정) 여부를 확인한다.
