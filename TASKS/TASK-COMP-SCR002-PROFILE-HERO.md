...
- **TASKS/00_TASK_LIST.md 참조:** Seq 14

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 14)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 대표 Hero은(는) SCR-002(`/about`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr002/ProfileHero.tsx`

## Functional AC

대표 사진+한 문장 소개, 실제 인물 오인 방지 원칙
...
- **TC-01:** 대표 사진+한 문장 소개 여부를 확인한다.
- **TC-02:** 실제 인물 오인 방지 원칙 여부를 확인한다.
