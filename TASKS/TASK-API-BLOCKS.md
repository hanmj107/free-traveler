...
- **TASKS/00_TASK_LIST.md 참조:** Seq 51

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 51)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 차단 생성/해제 API은(는) 여러 화면에 걸친 공통/백엔드 계층에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- **Screen:** N/A(화면 비종속)
- **Route:** N/A
- **Page Entry:** `src/app/api/blocks/route.ts`
...
상호 비노출 원칙 서버 측 강제(PRIV-004)

## Test Cases

- **TC-01:** 차단 생성/해제 여부를 확인한다.
- **TC-02:** 본인 관계만 생성 가능 여부를 확인한다.
