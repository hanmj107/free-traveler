...
- **Category:** AUTH (Auth — 인증/성인확인)
- **Priority:** P0
- **Implementation Status:** IN_SCOPE_PENDING
- **TASKS/00_TASK_LIST.md 참조:** Seq 47

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 47)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 성인 확인 로직은(는) 여러 화면에 걸친 공통/백엔드 계층에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.

## Project Scope

**IMPLEMENT** — `docs/06_SRS_UIUX_REVISED.md`, `docs/PROJECT_SCOPE.md`에서 이번 범위 구현 대상으로 확정됨.

`docs/PROJECT_SCOPE.md` §3 "인증·개인정보" 원칙: Supabase Auth(이메일)로 로그인/회원가입을 구현하고, 성인 확인은 생년월일을 저장하지 않는 자기신고 체크박스 + 확인 시각 저장 방식으로 구현한다.
...
- 해당 없음(비UI, 인증 계층). `docs/PROJECT_SCOPE.md` §3 "인증·개인정보" 원칙(Supabase Auth 이메일, 자기신고 성인확인)
...
- `src/lib/auth/adultVerification.ts`

## Functional AC

자기신고 체크박스 제출 시 `profiles.adult_verified_at`에 확인 시각만 저장, 생년월일 저장 금지, 미확인 사용자는 모집글 작성/참가요청 시 SCR-005로 리디렉션
...
- **TC-01:** 자기신고 체크박스 제출 시 `profiles.adult_verified_at`에 확인 시각만 저장 여부를 확인한다.
- **TC-02:** 생년월일 저장 금지 여부를 확인한다.
- **TC-03:** 미확인 사용자는 모집글 작성/참가요청 시 SCR-005로 리디렉션 여부를 확인한다.
