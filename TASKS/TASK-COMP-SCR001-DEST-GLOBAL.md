...
- **TASKS/00_TASK_LIST.md 참조:** Seq 8

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 8)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 해외 인기 여행지 카드 6개은(는) SCR-001(`/`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr001/GlobalDestinations.tsx`

## Functional AC

`region_type='global'` 데이터 6개 이상, 국가·도시명+안전등급 배지, 필터 AND 조건 동일 적용

## Visual AC

Card Grid 4열/1열, 안전등급 배지는 색상+텍스트 라벨 병기(ACC-004)
...
- **TC-01:** `region_type='global'` 데이터 6개 이상 여부를 확인한다.
- **TC-02:** 국가·도시명+안전등급 배지 여부를 확인한다.
- **TC-03:** 필터 AND 조건 동일 적용 여부를 확인한다.
