...
- **TASKS/00_TASK_LIST.md 참조:** Seq 25

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 25)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 동행 작성 Form(연락처 탐지+안전수칙)은(는) SCR-003(`/travel-tools`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr003/MateComposeForm.tsx`

## Functional AC

국가·지역·기간·스타일·소개글 필수 필드+안전수칙 동의 체크 후 제출 가능(MATE-003), 본문 내 전화번호·메신저 ID 패턴 탐지 시 제출 차단(MATE-009), 안전고지 문구 표시(PRIV-005)
...
본문 새니타이즈(SEC-008), 연락처 필드 비공개(PRIV-006)

## Test Cases

- **TC-01:** 국가·지역·기간·스타일·소개글 필수 필드+안전수칙 동의 체크 후 제출 가능(MATE-003) 여부를 확인한다.
- **TC-02:** 본문 내 전화번호·메신저 ID 패턴 탐지 시 제출 차단(MATE-009) 여부를 확인한다.
- **TC-03:** 안전고지 문구 표시(PRIV-005) 여부를 확인한다.
