...
- **TASKS/00_TASK_LIST.md 참조:** Seq 33

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 33)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 차단 UI은(는) SCR-004(`/mates`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr004/BlockButton.tsx`

## Functional AC

차단 설정 시 이후 목록·상세에서 상대방 콘텐츠 상호 비노출
...
상호 비노출 원칙(PRIV-004, RLS 연동)

## Test Cases

- **TC-01:** 차단 설정 시 이후 목록·상세에서 상대방 콘텐츠 상호 비노출 여부를 확인한다.
