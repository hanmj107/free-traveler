...
- **TASKS/00_TASK_LIST.md 참조:** Seq 62

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 62)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 접근성 수동 확인(브라우저)은(는) SCR-001~SCR-005에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `TASKS/checklists/manual-accessibility.md`

## Functional AC

5개 화면 전체 스크린리더 낭독, 키보드만으로 전체 흐름 조작, 색상 배지 옆 텍스트 라벨 존재, 터치 영역 44px 실측 — **브라우저 확인이 필요한 항목이므로 Manual Check Task로 연결**(원칙 4)
...
- **TC-01:** 5개 화면 전체 스크린리더 낭독 여부를 확인한다.
- **TC-02:** 키보드만으로 전체 흐름 조작 여부를 확인한다.
- **TC-03:** 색상 배지 옆 텍스트 라벨 존재 여부를 확인한다.
- **TC-04:** 터치 영역 44px 실측 — **브라우저 확인이 필요한 항목이므로 Manual Check Task로 연결**(원칙 4) 여부를 확인한다.
