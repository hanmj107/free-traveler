...
- **TASKS/00_TASK_LIST.md 참조:** Seq 53

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 53)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 날짜 검증 Unit Test은(는) SCR-003에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `tests/unit/travelDates.spec.ts`

## Functional AC

과거 출발일/체크인 차단, 귀국일<출발일 차단, 체크아웃≤체크인 차단 경계값(당일 포함) 테스트
...
- **TC-01:** 과거 출발일/체크인 차단 여부를 확인한다.
- **TC-02:** 귀국일<출발일 차단 여부를 확인한다.
- **TC-03:** 체크아웃≤체크인 차단 경계값(당일 포함) 테스트 여부를 확인한다.
- **TC-04:** 위 시나리오가 실패(회귀)할 경우 CI에서 실패로 표시되는지 확인한다.

## Verify

- 자체 검증(CI-PIPELINE-SETUP에서 자동 실행)
