...
- **TASKS/00_TASK_LIST.md 참조:** Seq 7

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 7)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 국내 인기 여행지 카드 6개은(는) SCR-001(`/`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr001/DomesticDestinations.tsx`

## Functional AC

`region_type='domestic'` 데이터 6개 이상을 클라이언트 필터링(AND 조건: 계절·테마·기간)으로 표시, 결과 0건 시 조건 완화 안내+초기화 버튼(DEST-004)

## Visual AC

Card Grid 4열(Desktop)/1열(Mobile), 사진 우선 카드, 카드 6개 미만 노출 금지
...
- **TC-01:** `region_type='domestic'` 데이터 6개 이상을 클라이언트 필터링(AND 조건: 계절·테마·기간)으로 표시 여부를 확인한다.
- **TC-02:** 결과 0건 시 조건 완화 안내+초기화 버튼(DEST-004) 여부를 확인한다.
