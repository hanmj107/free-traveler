...
- **Category:** UNIT_TEST (Unit Test)
- **Priority:** P0
- **Implementation Status:** IN_SCOPE_PENDING
- **TASKS/00_TASK_LIST.md 참조:** Seq 55

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 55)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 동행 상태 전이 Unit Test은(는) SCR-004에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `tests/unit/mateState.spec.ts`

## Functional AC

PENDING→APPROVED/REJECTED 전이, 중복 PENDING/APPROVED 차단, `end_date` 경과 시 마감 계산 경계값 테스트
...
- **TC-01:** PENDING→APPROVED/REJECTED 전이 여부를 확인한다.
- **TC-02:** 중복 PENDING/APPROVED 차단 여부를 확인한다.
- **TC-03:** `end_date` 경과 시 마감 계산 경계값 테스트 여부를 확인한다.
- **TC-04:** 위 시나리오가 실패(회귀)할 경우 CI에서 실패로 표시되는지 확인한다.

## Verify

- 자체 검증(CI-PIPELINE-SETUP)
