...
- **TASKS/00_TASK_LIST.md 참조:** Seq 46

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 46)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. Supabase Auth 이메일 로그인/가입 + 콜백은(는) 여러 화면에 걸친 공통/백엔드 계층에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
이메일 회원가입/로그인/세션 관리, `/auth/callback`은 기술 Route(디자인 Screen에 미포함)
...
HTTPS 전제(SEC-001), 비밀번호는 Supabase Auth 위임(자체 저장 없음)

## Test Cases

- **TC-01:** 이메일 회원가입/로그인/세션 관리 여부를 확인한다.
- **TC-02:** `/auth/callback`은 기술 Route(디자인 Screen에 미포함) 여부를 확인한다.
