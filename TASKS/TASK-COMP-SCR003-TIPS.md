...
- **TASKS/00_TASK_LIST.md 참조:** Seq 24

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 24)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 찾기 Tip 카드 3개은(는) SCR-003(`/travel-tools`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr003/TipCards.tsx`

## Functional AC

"환율은 출발 전 다시 확인하세요", "왕복보다 각각 검색하면 더 저렴할 수 있어요", "숙소는 위치와 후기를 함께 비교하세요" 3개 카드 + "입력하신 정보는 저장되지 않습니다" 고지 문구
...
- **TC-01:** "환율은 출발 전 다시 확인하세요" 여부를 확인한다.
- **TC-02:** "왕복보다 각각 검색하면 더 저렴할 수 있어요" 여부를 확인한다.
- **TC-03:** "숙소는 위치와 후기를 함께 비교하세요" 3개 카드 + "입력하신 정보는 저장되지 않습니다" 고지 문구 여부를 확인한다.
