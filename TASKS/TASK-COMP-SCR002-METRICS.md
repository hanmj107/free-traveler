...
- **TASKS/00_TASK_LIST.md 참조:** Seq 15

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 15)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 여행 지표 숫자 카드 2개은(는) SCR-002(`/about`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr002/MetricCards.tsx`

## Functional AC

"50+ Trips", "30+ Countries" 카드, PAGE-SCR001 요약과 동일 소스·동일 값
...
- **TC-01:** "50+ Trips" 여부를 확인한다.
- **TC-02:** "30+ Countries" 카드 여부를 확인한다.
- **TC-03:** PAGE-SCR001 요약과 동일 소스·동일 값 여부를 확인한다.
