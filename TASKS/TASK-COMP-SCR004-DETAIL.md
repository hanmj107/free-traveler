...
- **TASKS/00_TASK_LIST.md 참조:** Seq 30

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 30)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 상세 패널(Desktop 분할)/Drawer(Mobile)은(는) SCR-004(`/mates`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr004/MateDetailPanel.tsx`

## Functional AC

선택한 모집글 전체 내용, 마감 상태 계산 반영, 작성자 닉네임만 표시

## Visual AC

Desktop 좌우 분할 / Mobile 전체화면 Drawer
...
- **TC-01:** 선택한 모집글 전체 내용 여부를 확인한다.
- **TC-02:** 마감 상태 계산 반영 여부를 확인한다.
- **TC-03:** 작성자 닉네임만 표시 여부를 확인한다.
