...
- **TASKS/00_TASK_LIST.md 참조:** Seq 32

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 32)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 신고 UI(접수번호 표시)은(는) SCR-004(`/mates`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `design-reference/D-001/DESIGN.md` §15(Alert·Toast)
...
- `src/components/scr004/ReportDialog.tsx`

## Functional AC

신고 사유 제출 시 생성된 레코드 ID를 접수번호로 Toast/화면 표시
...
- **TC-01:** 신고 사유 제출 시 생성된 레코드 ID를 접수번호로 Toast/화면 표시 여부를 확인한다.
