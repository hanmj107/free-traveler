...
- **TASKS/00_TASK_LIST.md 참조:** Seq 52

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 52)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 외부 URL 설정 저장/조회 API은(는) 여러 화면에 걸친 공통/백엔드 계층에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- **Screen:** N/A(화면 비종속)
- **Route:** N/A
- **Page Entry:** `src/app/api/admin/external-urls/route.ts`
...
- `src/app/api/admin/external-urls/route.ts`

## Functional AC

관리자만 항공/호텔 랜딩 URL 저장, 일반 사용자는 조회만(SCR-003 Action Card가 이 값을 읽음)
...
- **TC-01:** 관리자만 항공/호텔 랜딩 URL 저장 여부를 확인한다.
- **TC-02:** 일반 사용자는 조회만(SCR-003 Action Card가 이 값을 읽음) 여부를 확인한다.
