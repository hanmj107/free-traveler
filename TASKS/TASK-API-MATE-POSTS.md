...
- **TASKS/00_TASK_LIST.md 참조:** Seq 48

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 48)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 모집글 생성/조회 API은(는) 여러 화면에 걸친 공통/백엔드 계층에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/app/api/mates/[id]/route.ts`

## Functional AC

공개·모집중 글만 비회원에게도 노출(연락처 필드 응답에서 제외), 생성 시 안전수칙 동의+연락처 미포함 검증, 마감 상태는 응답 시점 계산
...
서버 측 연락처 패턴 재검증(SEC-008), 공개 응답에 연락처 필드 없음(PRIV-006)

## Test Cases

- **TC-01:** 공개·모집중 글만 비회원에게도 노출(연락처 필드 응답에서 제외) 여부를 확인한다.
- **TC-02:** 생성 시 안전수칙 동의+연락처 미포함 검증 여부를 확인한다.
- **TC-03:** 마감 상태는 응답 시점 계산 여부를 확인한다.
