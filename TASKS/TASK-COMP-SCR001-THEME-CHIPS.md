...
- **TASKS/00_TASK_LIST.md 참조:** Seq 9

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 9)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 여행 동기·테마 Chip 6개은(는) SCR-001(`/`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr001/ThemeChips.tsx`

## Functional AC

휴양/도시 탐방/미식/자연·하이킹/가족여행/저예산 Chip 선택 시 위 두 목록에 필터 적용, 결과 0건 시 완성형 Empty State

## Visual AC

Chip pill 형태, 선택 상태 코랄 배경+흰 텍스트, 터치 영역 44px 이상(ACC-006)
...
- **TC-01:** 휴양/도시 탐방/미식/자연·하이킹/가족여행/저예산 Chip 선택 시 위 두 목록에 필터 적용 여부를 확인한다.
- **TC-02:** 결과 0건 시 완성형 Empty State 여부를 확인한다.
