...
- **TASKS/00_TASK_LIST.md 참조:** Seq 26

## Context

이 Task는 `TASKS/00_TASK_LIST.md`(Seq 26)에 정의된 구현 Task를 실제로 개발 가능한 단위로 상세화한 것이다. 로그인 유도 카드 + 안전 안내 패널은(는) SCR-003(`/travel-tools`)에서 사용되며, `docs/06_SRS_UIUX_REVISED.md`가 확정한 Screen/Route 배치를 따른다.
...
- `src/components/scr003/LoginPromptCard.tsx`

## Functional AC

비로그인/성인 미확인 상태에서 동행 구하기 탭 진입 시 작성 폼 대신 로그인 유도 카드+안전 안내 사이드 패널 표시, 작성 폼은 렌더링하지 않음

## Visual AC

좌: 로그인 유도, 우: 안전 안내(Desktop 좌우 분할)
...
- **TC-01:** 비로그인/성인 미확인 상태에서 동행 구하기 탭 진입 시 작성 폼 대신 로그인 유도 카드+안전 안내 사이드 패널 표시 여부를 확인한다.
- **TC-02:** 작성 폼은 렌더링하지 않음 여부를 확인한다.
