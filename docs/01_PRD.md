...
- **작성일:** 2026-08-20
- **최종 수정일:** 2026-09-03 (SRS 검토 결과 Critical/Major 항목 일부 반영 — 변경 내역은 §13 참조)
- **상태:** Approved for SRS
- **제품 대표:** `free_traveler`
...
| **AC-M02** | 사용자(비회원 포함) | 국가·지역·기간·여행 스타일 필터 | 조건과 겹치는 공개·모집중 글 목록을 작성자 연락처 등 개인정보 노출 없이 표시 | p95 1초 이내 |
...
> 모집글·참가 요청 **목록·상세 열람**은 비회원도 가능하다(단, 작성자 연락처 등 개인정보는 열람 권한과 무관하게 노출하지 않는다). 글 작성·참가 요청·신고·차단은 AC-M01에 따라 로그인과 성인 확인이 모두 필요하다. (2026-09-03 수정: SRS 검토에서 발견된 §3 Stakeholders·API 스펙 간 불일치를 해소하기 위해 AC-M02의 Given을 "성인 인증 회원"에서 "사용자(비회원 포함)"로 수정함)
...
> Supabase는 위 표에 함께 정리했으나 시스템 경계상 내부 인프라로 취급하며, 항공·호텔·외교부·TourAPI와 같은 "외부 인터페이스 실패 처리" 대상에는 포함하지 않는다. 다만 §7-1 가용성 목표(월간 99.5% 이상)는 Supabase 가용성에 의존하므로, Supabase 장애 시에도 여행지·안전정보 등 읽기 전용 콘텐츠는 가능한 범위에서 열람 가능하도록 한다. (2026-09-03 추가)
...
| **D-02** | 이메일 알림 서비스 사용 가능 (Should 우선순위 "동행 요청 알림 이메일" 착수 시 필요 — MVP Must 범위의 필수 의존성은 아님, 2026-09-03 명확화) | 참가 요청 알림 PoC |
...
| 일자 | 변경 위치 | 변경 내용 | 근거 |
|---|---|---|---|
| 2026-09-03 | Story 4, AC-M02 | Given을 "성인 인증 회원" → "사용자(비회원 포함)"로 수정, Then에 개인정보 비노출 조건 추가 | `SRS_Drafts/04_SRS_Travel_review.md` Critical-1: §3 Stakeholders(비회원 열람 허용)와 AC-M02(성인 인증 회원 한정) 간 충돌 |
| 2026-09-03 | §8-2 외부 연결 | Supabase의 내부/외부 분류 및 장애 시 최소 대응 방향을 각주로 명확화 | 동 리뷰 §13 충돌 C6, Major(Supabase 장애 대응 요구사항 부재) |
| 2026-09-03 | §10-2 D-02 | 이메일 알림 서비스 의존성이 Should 기능 착수 시에만 필요함을 명시 | 동 리뷰 §13 충돌 C4(이메일 알림 의존성 표현 불일치) |

> 이번 개정은 SRS 검토(`SRS_Drafts/04_SRS_Travel_review.md`)에서 식별된 Critical·Major 항목 중 PRD 수정이 선행되어야 하는 항목만 반영했다. 그 외 Major/Minor 항목은 SRS 쪽 수정만으로 해소했다(해당 SRS 개정 이력 참조).
